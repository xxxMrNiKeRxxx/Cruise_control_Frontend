...
export interface DrivingMode { // заменено с Tire на DrivingMode
  mode_id: number; // заменено с tire_id
  mode_name: string; // заменено с tire_title
  description: string;
  image_key?: string; // заменено с photo
  video_key?: string; // заменено с video
  base_consumption: number; // заменено с tire_material_coefficient
  economy_percent: number; // новое поле для экономии
  driving_type: 'city' | 'highway' | 'mixed'; // тип движения
  price: number; // цена
  // short_description_en — можно оставить, если используется
...
export async function getFuelConsumptionCart(): Promise<FuelConsumptionCart> {
  try {
    const res = await fetch("/api/fuel_consumption/fuel-consumption-cart", { // изменён URL
